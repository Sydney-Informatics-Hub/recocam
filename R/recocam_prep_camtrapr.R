...
#' @param number_of_species An integer specifying how many species could be tagged in a single image. This integer will be used to help specify the number of columns that we want the function to work on.
...
prep_camtrapr <- function(df_fixed,
  ...
  if (wildcount == FALSE){
    ...
    species_data <- df_fixed %>%
      dplyr::select(file_name, species, second_species, third_species_group, everything()) %>%
      rename("species_1" = species,
             "species_2" = second_species,
             "species_3" = third_species_group)
