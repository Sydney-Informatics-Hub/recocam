...
**recocam** provides robust tools for extracting and processing information and annotations that are stored in image EXIF metadata. While this package provides tools that are useful for any images, it includes several functions that are optimized to help ecolgoists prepare manually tagged camera trap images for analysis with the **camtrapR** package.
...
Currently, you need to install **devtools** before you can install this package.
```{r package installing, eval=FALSE}
...
keywords_example <- read_csv(system.file("extdata/keywords.csv", package="recocam"), col_names = TRUE)
